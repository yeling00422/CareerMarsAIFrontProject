...
export default {
  ...
  methods: {
    ...
    async loadData() {
      ...
      try {
        const response = await api.post('/ai/recommendation/mentor', {
        ...
        if (result.code === 200) {
          ...
          this.$router.push({
            path: '/end-result',
            query: { teachers: JSON.stringify(result.data.mentorList), token },
          });
        } else {
          MessageBox.confirm(result.msg || '推荐导师生成失败，是否重新生成？', '提示', {
            confirmButtonText: '重新生成推荐导师',
    ...
    startProgressAnimation() {
      ...
      this.progressTimer = setInterval(() => {
        if (this.progress < 90) this.progress += Math.random() * 20;
      }, 500);
    },
