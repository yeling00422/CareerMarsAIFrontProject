...
export default {
  ...
  methods: {
    loadAnalysisData() {
      ...
      const analysisResultData = this.$route.query.analysisResultData;// 将数据转为字符串传递
      ...
      console.log("analysis-result-analysisResultData:",analysisResultData);
      ...
      if (analysisResultData) {
        try {
          const resultData = JSON.parse(analysisResultData);
