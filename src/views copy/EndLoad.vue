...
export default {
  ...
  methods: {
    async loadAnalysisData() {
      // 从路由参数中获取数据
      // const teachers = this.$route.query.teachers;// 将数据转为字符串传递
      // console.log("end-load-teachers:",teachers);
      const resumeText = this.$route.query.resumeText;// 将数据转为字符串传递
      const position = this.$route.query.position;// 将数据转为字符串传递
      ...
      // 设置延迟3秒
      setTimeout(() => {
        this.$router.push({path: '/end-result',
        query: {
          teachers: teachers,
        }});
      }, 3000); // 3000毫秒 = 3秒
